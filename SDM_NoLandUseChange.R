...
points(occurrence_lonlat$lon,occurrence_lonlat$lat, col='black',pch=3)
...
# if already run, read ensemble_raster
...
predictors_LU_in <- raster('C:/Users/Vivienne Groner/Desktop/Vivienne/2020/SDM/Clivia_miniata_2020/predictors/MODIS/MODIS_LCType.grd')
predictors_LU <- crop(predictors_LU_in,extent(ensemble_raster))
plot(predictors_LU)

# extract land cover type at occurrence, just do check
extract_LCtype<-table(extract(predictors_LU,occurrence_lonlat))
extract_LCtype
...
writeRaster(lct_mask,file=paste(outdir_LU,species_name,'_LUonly_HS.asc',sep=''),overwrite=TRUE)

predictor_LU_masked<-list()
for (i in 1:length(thres_list)){
# mask land use with occurrence based on climate  
   ensemble_raster_mask<- ensemble_raster
   ensemble_raster_mask[ensemble_raster_mask <tr_ensemble[[i]]] <- NA
   predictor_LU_masked[[i]] <- mask(predictors_LU_species,ensemble_raster_mask)  
   saveRDS(predictor_LU_masked[[i]], file = paste(outdir_LU,species_name,"_pa_",thres_list[[i]],"LU.asc",sep=''),ascii=TRUE)
}

predictor_LU_masked_stack<-stack(predictor_LU_masked)

tiff(paste(outdir_LU,species_name,"_pa_LUonly.tiff", sep=''))
plot(predictor_LU_masked_stack[[3]],legend = FALSE, col = rev(terrain.colors(2)), main=species_name,xlab="longitude", ylab="latitude")
box()
points(occurrence_in$lon,occurrence_in$lat,pch=3) 
dev.off()
...
clusterApply(cl, 1:nrow(all_proj), function(pn){
  ...
  proj_out[[pn]]<-proj_out[[pn]]*predictors_LU_species
